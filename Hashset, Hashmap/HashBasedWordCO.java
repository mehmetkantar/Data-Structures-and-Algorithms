  ...
  public HashSet<String> getUniqueWords(String text) {
    ...
    for (String sentence : sentences) {
      for (String word : sentence.split("\\s+")) {
        ...
        // the minWordLen
    	  if (word.length() >= minWordLen && !ignoreSet.contains(word))
              uWords.put(word);
      }
    }
    return uWords;
  ...
  public void fillCoMat(String text, String[] keyWords) {
    ...
    for (String s : sentences) {
        ...
        for (int i = 0; i < sWords.length; i++) {
            String cWord = sWords[i];
            if (!words.contains(cWord))
                continue;

            for (int j = i - windowSize; j <= i + windowSize; j++) {
                if (j >= 0 && j < sWords.length && i != j && words.contains(sWords[j]))
                    coMat.get(cWord).increment(sWords[j]);
            }
        }
    }
  }
